...
class HentaiController {
    ...
    static async deleteHentai(req, res) {
        const { id } = req.params;
        ...
        try {
            ...
            if (hentai.image) {
                ...
                s3.deleteObject(paramsImage, (err, data) => {
                    if (err) {
                        console.log("Erro ao excluir a imagem:", err);
                    }
                    else {
                        console.log("Imagem excluída da AWS S3 com sucesso!");
                    }
                });
            }
                ...
                for (const chapter of hentai.chapters) {
                    if (chapter.imagesChapter &&
                        ...
                        for (const imageChapter of chapter.imagesChapter) {
                            ...
                            s3.deleteObject(paramsChapter, (err, data) => {
                                if (err) {
                                    console.log("Erro ao excluir a imagem do capítulo:", err);
                                }
                                else {
                                    console.log("Imagem do capítulo excluída da AWS S3 com sucesso!");
                                }
                            });
                        }
                    }
